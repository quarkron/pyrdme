#!/usr/bin/env python3
"""
Sweep N_RNAP × N_ATP at fixed N_Ribo.

Runs parameter sweeps for the autocatalytic loop collapse experiments
and saves all results to the results/ folder.

Usage:
    python sweep_rnap_atp.py [--n-grid N] [--n-seeds M] [--fixed-ribo R] [--output-dir DIR]
    python sweep_rnap_atp.py --resume results/sweep_XXXXXXXX_XXXXXX [--n-grid N] [--n-seeds M]
"""
...
def run_ensemble_with_progress(func, param_grid, sweep_state=None):
    ...
    for i, params in enumerate(param_grid, 1):
        ...
        print(f"[{i:>5}/{total}] {pct:>5.1f}% | "
              f"P={params['N_RNAP']:>3} ATP={params['N_ATP']:>5} → {mode:15s} "
...
def plot_phase_diagrams(rgb, mode_grid, t_death, wd, mode_names, pnr_grid, pnr_names,
                        ...
                        x_label='Initial RNAP', y_label='Initial ATP',
                        title_suffix=''):
...
def _validate_resume_config(meta, RNAP_AXIS, RIBO_AXIS, ATP_AXIS, n_seeds, t_max):
    ...
    mismatches = []
    if meta.get('RNAP_AXIS') != RNAP_AXIS.tolist():
        mismatches.append(
            f"  RNAP_AXIS: saved={meta.get('RNAP_AXIS')} vs current={RNAP_AXIS.tolist()}")
    if meta.get('RIBO_AXIS') != RIBO_AXIS:
        mismatches.append(
            f"  RIBO_AXIS: saved={meta.get('RIBO_AXIS')} vs current={RIBO_AXIS}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Sweep N_RNAP × N_ATP at fixed N_Ribo'
    )
    parser.add_argument('--n-grid', type=int, default=8,
                        help='Grid points per RNAP axis (default: 8)')
    ...
    parser.add_argument('--fixed-ribo', type=int, default=20,
                        help='Fixed ribosome count (default: 20)')
    ...
    if args.resume:
        ...
        output_dir = Path(args.output_dir) / f'sweep_rnap_atp_{sweep_timestamp}'

        state = SweepState(output_dir)
        state.save_metadata(
            RNAP_AXIS=RNAP_AXIS.tolist(),
            RIBO_AXIS=RIBO_AXIS,
        ...
        print("=" * 60)
        print("RNAP × ATP Sweep (fixed N_Ribo)")
        print("=" * 60)
        print(f"Output directory: {output_dir}")

    print(f"RNAP grid: {len(RNAP_AXIS)} points")
    print(f"ATP grid:  {len(ATP_AXIS)} points")
    print(f"Seeds per point: {args.n_seeds}")
    print(f"Fixed Ribo: {args.fixed_ribo}")
    ...
    rgb, mode_grid, t_death, wd, mode_names, pnr_grid, pnr_names = aggregate_results(
        results, 'N_RNAP', 'N_ATP', RNAP_AXIS, ATP_AXIS
    )

    plot_phase_diagrams(rgb, mode_grid, t_death, wd, mode_names, pnr_grid, pnr_names,
                        RNAP_AXIS, ATP_AXIS, output_dir,
                        x_label='Initial RNAP', y_label='Initial ATP',
                        title_suffix=f', N_Ribo = {args.fixed_ribo}')
    ...
    np.savez(aggregate_file,
             ...
             pnr_grid=pnr_grid,
             RNAP_AXIS=RNAP_AXIS, ATP_AXIS=ATP_AXIS)
    print(f"Saved aggregated data: {aggregate_file}")

    # Save summary statistics
    summary_file = output_dir / 'summary.txt'
    with open(summary_file, 'w') as f:
        f.write("RNAP × ATP Sweep Summary (fixed N_Ribo)\n")
        f.write("=" * 60 + "\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"RNAP grid: {len(RNAP_AXIS)} points, ATP grid: {len(ATP_AXIS)} points\n")
        f.write(f"Fixed Ribo: {args.fixed_ribo}\n")
