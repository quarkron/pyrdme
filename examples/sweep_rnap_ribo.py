#!/usr/bin/env python3
"""
Sweep N_RNAP × N_Ribo at fixed N_ATP.

Runs parameter sweeps for the autocatalytic loop collapse experiments
and saves all results to the results/ folder.

Usage:
    python sweep_rnap_ribo.py [--n-grid N] [--n-seeds M] [--fixed-atp A] [--output-dir DIR]
    python sweep_rnap_ribo.py --resume results/sweep_XXXXXXXX_XXXXXX [--n-grid N] [--n-seeds M]
"""
...
def run_ensemble_with_progress(func, param_grid, sweep_state=None):
    ...
    for i, params in enumerate(param_grid, 1):
        ...
        print(f"[{i:>5}/{total}] {pct:>5.1f}% | "
              f"P={params['N_RNAP']:>3} R={params['N_Ribo']:>3} → {mode:15s} "
...
def plot_phase_diagrams(rgb, mode_grid, t_death, wd, mode_names, pnr_grid, pnr_names,
                        ...
                        x_label='Initial RNAP', y_label='Initial Ribosome',
                        title_suffix=''):
...
def _validate_resume_config(meta, RNAP_AXIS, RIBO_AXIS, ATP_AXIS, n_seeds, t_max):
    ...
    if meta.get('ATP_AXIS') != ATP_AXIS:
        mismatches.append(
            f"  ATP_AXIS: saved={meta.get('ATP_AXIS')} vs current={ATP_AXIS}")
...
def main():
    ...
    parser.add_argument('--fixed-atp', type=int, default=1000,
                        help='Fixed ATP count (default: 1000)')
    ...
    # Generate axes (needed for both fresh and resume paths)
    ...
    if args.resume:
        ...
        print("=" * 60)
        print("Spore Dormancy Phase 1 - RESUMING Parameter Sweep")
        print("=" * 60)
        ...
        output_dir = Path(args.output_dir) / f'sweep_{sweep_timestamp}'
    ...
    plot_phase_diagrams(rgb, mode_grid, t_death, wd, mode_names, pnr_grid, pnr_names,
                        ...
                        title_suffix=f', N_ATP = {ATP_AXIS[0]}')
